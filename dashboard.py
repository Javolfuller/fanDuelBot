...
Users can select a specific player and view the model’s performance across different prop markets (points, assists, rebounds, threes, blocks, steals) over the past month.
...
if view == "Monthly :chart_with_upwards_trend:":
	if player_select:
		...
		with col1:
			...
			if df_points.empty:
				...
				points_accuracy = round(df_points['predictionCorrect'].sum()/len(df_points), 2)

			# Set Charts
			st.write("Points predictions")
			st.line_chart(df_points[['eventDate', 'predictedStat', 'outcomeStat', 'line']], x='eventDate')
		...
		with col2:
			...
			if df_threes.empty:
				...
				threes_accuracy = round(df_threes['predictionCorrect'].sum()/len(df_threes), 2)

			# Set Charts
			st.write("Threes Made predictions")
			st.line_chart(df_threes[['eventDate', 'predictedStat', 'outcomeStat', 'line']], x='eventDate')
		...
		with col3:
			...
			if df_assists.empty:
				...
				assists_accuracy = round(df_assists['predictionCorrect'].sum()/len(df_assists), 2)

			# Set Charts
			st.write("Assists predictions")
			st.line_chart(df_assists[['eventDate', 'predictedStat', 'outcomeStat', 'line']], x='eventDate')
		...
		with col4:
			...
			if df_rebounds.empty:
				...
				rebounds_accuracy = round(df_rebounds['predictionCorrect'].sum()/len(df_rebounds), 2)

			# Set Charts
			st.write("Rebounds predictions")
			st.line_chart(df_rebounds[['eventDate', 'predictedStat', 'outcomeStat', 'line']], x='eventDate')
		...
		with col5:
			...
			if df_blocks.empty:
				...
				blocks_accuracy = round(df_blocks['predictionCorrect'].sum()/len(df_blocks), 2)

			# Set Charts
			st.write("Blocks predictions")
			st.line_chart(df_blocks[['eventDate', 'predictedStat', 'outcomeStat', 'line']], x='eventDate')
		...
		with col6:
			...
			if df_steals.empty:
				...
				steals_accuracy = round(df_steals['predictionCorrect'].sum()/len(df_steals), 2)

			# Set Charts
			st.write("Steals predictions")
			st.line_chart(df_steals[['eventDate', 'predictedStat', 'outcomeStat', 'line']], x='eventDate')
	...
	if player_select:
		...
		if date_select:
			...
			st.bar_chart(filterd_date_df, x='market', y=['predictedStat', 'outcomeStat'], stack=False)
