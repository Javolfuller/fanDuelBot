...
def refine_nba_stats(player_stats, team_stats, cut_off=5):
	...
	# Filter rolling year
	player_stats = player_stats[player_stats['gameDateLocal_year'] >= analysis_start_year]
	# Split dataframe for players whom total games in analysis year range meets 150 game or more
	total_games_last_nth_years_player_id_list = player_stats.groupby('personId')['personId'].count()
	total_games_last_nth_years_meets_threshold_player_id_list = total_games_last_nth_years_player_id_list[
	                                                            total_games_last_nth_years_player_id_list >= 150]

	# The average games played in the dataset meets threshold
	refined_player_stats_data = player_stats[player_stats['personId'].isin(
	                            total_games_last_nth_years_meets_threshold_player_id_list.index)].copy()

	# Create a full name ID to map player prop odds to player stats DF down the line
	# "P.J. Washington" → "PJWASHINGTON"
	refined_player_stats_data['fullNameId'] = refined_player_stats_data.apply(lambda x: x['firstName'].upper().replace('.', '').replace(' ', '') +
	                                          x['lastName'].upper().replace('.', '').replace(' ', ''), axis=1)
	# Filter Pre-season stats
	refined_player_stats_data = refined_player_stats_data[refined_player_stats_data['gameType'] != 'Preseason'].copy()
	# Create Next game date field
	refined_player_stats_data = refined_player_stats_data.sort_values(['personId', 'gameDateUTC'])
	refined_player_stats_data['nextGameId'] = refined_player_stats_data.groupby('personId')['gameId'].shift(-1)
	refined_player_stats_data['nextGameDateUTC'] = refined_player_stats_data.groupby('personId')['gameDateUTC'].shift(-1)
	refined_player_stats_data['nextGameDateUTC_date'] = refined_player_stats_data.groupby('personId')['gameDateUTC_date'].shift(-1)
	refined_player_stats_data['nextGameDateUTC_time'] = refined_player_stats_data.groupby('personId')['gameDateUTC_time'].shift(-1)
	refined_player_stats_data['nextGameDateUTC_hour'] = refined_player_stats_data.groupby('personId')['gameDateUTC_hour'].shift(-1)

	# create Next Game Stats
	refined_player_stats_data['nextGamePoints'] = refined_player_stats_data.groupby('personId')['points'].shift(-1)
	refined_player_stats_data['nextGameAssists'] = refined_player_stats_data.groupby('personId')['assists'].shift(-1)
	refined_player_stats_data['nextGameThreePointersMade'] = refined_player_stats_data.groupby('personId')['threePointersMade'].shift(-1)
	refined_player_stats_data['nextGameReboundsTotal'] = refined_player_stats_data.groupby('personId')['reboundsTotal'].shift(-1)
	refined_player_stats_data['nextGameSteals'] = refined_player_stats_data.groupby('personId')['steals'].shift(-1)
	refined_player_stats_data['nextGameBlocks'] = refined_player_stats_data.groupby('personId')['blocks'].shift(-1)


	# 2 - Prep team bosxore stats
	team_stats_for_merge = team_stats[['gameId', 'teamName', 'teamScore', 'reboundsTotal', 'fieldGoalsAttempted',
                                   'reboundsOffensive', 'freeThrowsAttempted', 'turnovers',
                                   'numMinutes']].rename(columns={'teamName': 'playerteamName',
	...
	opp_team_stats_for_merge = team_stats[['gameId', 'teamName', 'teamScore', 'reboundsTotal', 'reboundsOffensive', 'reboundsDefensive',
	                                       'blocks', 'steals', 'fieldGoalsAttempted', 'fieldGoalsMade', 'fieldGoalsPercentage', 'turnovers',
	                                       'freeThrowsAttempted','plusMinusPoints']].rename(
	                                       	columns={'teamName': 'opponentteamName',
	...
	refined_player_stats_data = refined_player_stats_data.merge(team_stats_for_merge, on=['gameId', 'playerteamName'])
	refined_player_stats_data = refined_player_stats_data.merge(opp_team_stats_for_merge, on=['gameId', 'opponentteamName'])

	# 3- compute advanced stats
	# Compute Advanced Stats

	refined_player_stats_data['trueShooting'] = refined_player_stats_data['points']/ (2 * (refined_player_stats_data['fieldGoalsAttempted'] +
	                                           (.44 * refined_player_stats_data['freeThrowsAttempted'])))

	refined_player_stats_data['effectiveFieldGoal'] = ((refined_player_stats_data['fieldGoalsMade'] + 0.5 *
	                                                   refined_player_stats_data['threePointersMade']) /
	                                                 refined_player_stats_data['fieldGoalsAttempted'])

	refined_player_stats_data['offensiveReboundingRate'] = 100 * (refined_player_stats_data['reboundsOffensive'] *
	...
	refined_player_stats_data['totalReboundingRate'] = 100 * (refined_player_stats_data['reboundsTotal'] *
	                                (refined_player_stats_data['playerteamNumMinutes'] / 5)) / (
	                                 refined_player_stats_data['numMinutes'] *
	                                (refined_player_stats_data['playerteamTotalRebounds'] +
	                                 refined_player_stats_data['opponentTeamReboundsTotal']))

	refined_player_stats_data['usageRate'] = 100 * (
	                                        (refined_player_stats_data['fieldGoalsAttempted'] +
	...
	refined_player_stats_data['opponentTeamTotalReboundingRate'] = 100 * (refined_player_stats_data['opponentTeamReboundsTotal'] /
	                                                                      (refined_player_stats_data['playerteamTotalRebounds'] +
	                                                                    refined_player_stats_data['opponentTeamReboundsTotal']))
	...
	refined_player_stats_data['opponentDefensiveRating'] = (100 * (refined_player_stats_data['playerteamTeamScore'] /
	                                                              refined_player_stats_data['oppEstimatedPosessions']))


	# fill Nas with 0s
	refined_player_stats_data['trueShooting'] = refined_player_stats_data['trueShooting'].fillna(0)
	refined_player_stats_data['effectiveFieldGoal'] = refined_player_stats_data['effectiveFieldGoal'].fillna(0)
	refined_player_stats_data['totalReboundingRate'] = refined_player_stats_data['totalReboundingRate'].fillna(0)
	refined_player_stats_data['usageRate'] = refined_player_stats_data['usageRate'].fillna(0)
	refined_player_stats_data['opponentTeamTotalReboundingRate'] = refined_player_stats_data['opponentTeamTotalReboundingRate'].fillna(0)
	refined_player_stats_data['opponentDefensiveRating'] = refined_player_stats_data['opponentDefensiveRating'].fillna(0)
	refined_player_stats_data['opponentTeamPlusMinusPoints'] = refined_player_stats_data['opponentTeamPlusMinusPoints'].fillna(0)
	refined_player_stats_data['plusMinusPoints'] = refined_player_stats_data['plusMinusPoints'].fillna(0)
	...
	refined_player_stats_data['seriesGameNumber'] = refined_player_stats_data['seriesGameNumber'].fillna(0)


	# 4 - Trim columns
	refined_player_stats_data = refined_player_stats_data[['firstName', 'lastName', 'fullNameId', 'personId', 'gameId', 'gameDate',
                           'gameDateLocal', 'gameDateUTC', 'gameDateLocal_date', 'gameDateLocal_time', 'gameDateLocal_year',
                           'gameDateUTC_date', 'gameDateUTC_time','gameDateUTC_hour', 'nextGameId', 'nextGameDateUTC', 'nextGameDateUTC_date',
                           'nextGameDateUTC_time', 'nextGameDateUTC_hour', 'playerteamCity', 'playerteamName',
                           'opponentteamCity', 'opponentteamName', 'gameType', 'gameLabel', 'gameSubLabel', 'seriesGameNumber', 'win',
                           'home', 'numMinutes', 'points', 'assists', 'blocks', 'steals', 'fieldGoalsAttempted', 'fieldGoalsMade',
                           'fieldGoalsPercentage', 'threePointersAttempted', 'threePointersMade', 'threePointersPercentage',
                           'freeThrowsAttempted', 'freeThrowsMade', 'freeThrowsPercentage', 'reboundsDefensive', 'reboundsOffensive',
                           'reboundsTotal', 'foulsPersonal', 'turnovers', 'plusMinusPoints','usageRate', 'trueShooting',
                           'effectiveFieldGoal', 'offensiveReboundingRate', 'totalReboundingRate', 'playerteamfieldGoalsAttempted',
                           'playerteamfreeThrowsAttempted', 'playerteamTurnovers', 'opponentTeamBlocks', 'opponentTeamSteals',
                           'opponentTeamPlusMinusPoints', 'opponentTeamReboundsOffensive', 'opponentTeamReboundsDefensive',
                           'opponentTeamTotalReboundingRate', 'opponentDefensiveRating', 'opponentTeamFieldGoalsAttempted',
                           'opponentTeamFieldGoalsMade', 'opponentTeamFieldGoalsPercentage', 'nextGamePoints', 'nextGameAssists',
                           'nextGameThreePointersMade', 'nextGameReboundsTotal', 'nextGameSteals', 'nextGameBlocks'
                           ]]

	return refined_player_stats_data


def build_features(df, stats_cols, event_date, sma_windows=[5, 10], ema_windows=[5, 10]):
    """
    ...
    for stat in stats_cols:
        ...
        for w in ema_windows:
            df[f'{stat}_EMA{w}'] = (
                df.groupby('fullNameId')[stat]
                .transform(lambda x: x.shift().ewm(span=w, adjust=False).mean())
            )
            # fill na with 0 for these stats
...
def get_current_events(api_key, sport):
    ...
    print("Remaining Requests for the Month: {}".format(response.headers.get("x-requests-remaining")))
    events_data = pd.DataFrame(events)

    if not events_data.empty:
	    # Create game date and time column
	    events_data['event_date'] = pd.to_datetime(events_data['commence_time'], utc=True).dt.date
	    events_data['event_time'] = pd.to_datetime(events_data['commence_time'], utc=True).dt.time
	    events_data['commence_time'] = pd.to_datetime(events_data['commence_time'], utc=True)

	    # Save current events

	    file_path = os.path.join("odds", "daily", "events", f"daily_events_{sport}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.parquet")
...
def get_current_player_props(df, sport, api_key):
    ...
    for EVENT_ID in df['id']:
        ...
        player_props_url = f"https://api.the-odds-api.com/v4/sports/{sport}/events/{EVENT_ID}/odds"
        player_props_params = {
            "apiKey": api_key,
	                ...
	                for market in bookmaker["markets"]:
	                    for outcome in market["outcomes"]:
	                        rows.append({
	                            ...
	                            "eventDateTime": pd.to_datetime(game["commence_time"], utc=True),
	                            "eventDate": pd.to_datetime(game["commence_time"], utc=True).date(),
	                            "eventTime": pd.to_datetime(game["commence_time"], utc=True).time(),
	                            "eventDateTime_Hour": pd.to_datetime(game["commence_time"], utc=True).hour,
	                            "timeStamp": market['last_update'],
	                            "player": outcome["description"],
	                            "fullNameId": (outcome["description"].split(' ')[0].upper().replace('.', '') +
	                                           "".join(outcome["description"].split(' ')[1:]).upper().replace('.', '')),
    ...
    if not player_props_data.empty:
	    ...
	    player_props_data['awayTeam'] = player_props_data['awayTeam'].apply(lambda x: ' '.join(x.split(' ')[-2:]) if 'Trail' in x else ''.join(x.split(' ')[-1:]))
	    player_props_data['homeTeam'] = player_props_data['homeTeam'].apply(lambda x: ' '.join(x.split(' ')[-2:]) if 'Trail' in x else ''.join(x.split(' ')[-1:]))

	    print("Remaining Requests for the Month: {}".format(response.headers.get("x-requests-remaining")))

	    # Save current player props
	    file_path = os.path.join( "odds", "daily", "player_props",
...
def create_historical_player_props_df(player_props, sport):
    ...
    historical_player_props_data = historical_player_props_data.sort_values('timeStamp', ascending=False)\
                                .drop_duplicates(['bookMaker', 'eventId',
                                                  'player', 'fullNameId', 'firstName',
                                                  'lastName', 'market', 'overUnder'])

    # Adjust team names to match kaggle datasets
    historical_player_props_data['awayTeam'] = historical_player_props_data['awayTeam'].apply(lambda x: ' '.join(x.split(' ')[-2:]) if 'Trail' in x else ''.join(x.split(' ')[-1:]))
    historical_player_props_data['homeTeam'] = historical_player_props_data['homeTeam'].apply(lambda x: ' '.join(x.split(' ')[-2:]) if 'Trail' in x else ''.join(x.split(' ')[-1:]))

    # Save props
    historical_player_props_data.to_parquet(
        'odds/historical/player_props/historical_player_props_{}_{}.parquet'.format(sport, datetime.now().strftime('%Y%m%d_%H%M%S')),
        engine="pyarrow", index=False)

    return historical_player_props_data


def fetch_historical_props_retry(event_id, commence_time, api_key, sport, bookmaker="fanduel", max_retry_hours=24, markets=None):
    """
    ...
    """
    if markets is None:
        markets = ["player_points", "player_threes", "player_assists",
                   "player_rebounds", "player_blocks", "player_steals"]
...
def get_historical_historical_events(api_key, sport, start_date, end_date):
    ...
    json_file_path = 'odds/historical/events/historical_events_{}_{}.json'.format(sport, datetime.now().strftime("%Y%m"))
    ...
    while historical_current_date <= end_date:
        ...
        historical_events_url = f"https://api.the-odds-api.com/v4/historical/sports/{sport}/events"
        historical_events_params = {"apiKey": api_key, "date": date_str}

        historical_response = requests.get(historical_events_url, params=historical_events_params)
    ...
    print("Remaining Requests for the Month: {}".format(historical_response.headers.get("x-requests-remaining")))


    historical_events_data['event_date'] = pd.to_datetime(historical_events_data['commence_time'], utc=True).dt.date
    historical_events_data['event_time'] = pd.to_datetime(historical_events_data['commence_time'], utc=True).dt.time
    historical_events_data['commence_time'] = pd.to_datetime(historical_events_data['commence_time'], utc=True)
    ...
    historical_events_data = historical_events_data[mask].copy().sort_values('commence_time')
...
def get_historical_player_props(df, api_key, sport):
    ...
    file_path = 'odds/historical/player_props/historical_player_props_{}_{}.json'.format(
    ...
    for _, historical_game in df.iterrows():
        ...
        historical_odds_date = pd.to_datetime(historical_game['commence_time'], utc=True)
        historical_odds_date_str = historical_odds_date.replace(
            hour=23, minute=59, second=59
        ).isoformat().replace("+00:00", "Z")
        print("Searching for Players props for {}, event id:".format(historical_odds_date_str, historical_game['id']))
        print("")
        ...
        historical_response = requests.get(historical_event_url, params=historical_params)

        if historical_response.status_code == 200:
            result = historical_response.json()
            historical_player_props.append(result)
            print("Remaining Requests for the Month:",
            ...
            if retry_fetch is not None:
                ...
                print(f"Error for event {historical_game['id']} at {historical_odds_date_str}: {historical_response.text}")
...
def clean_props_with_boxscore(df, minuts_played_col='numMinutes', game_type_col=None):
	...
	if game_type_col != None:
		...
		encoder_path = os.path.join(os.path.dirname(__file__), "..", "models/basketball_nba/nba_game_type_label_encoder.pkl")
...
def ml_feature_selection(sport='basketball_nba', market=None):
	...
	if sport == 'basketball_nba':
		if market == 'player_points':
			features = ['points_EMA10', 'points_SMA10', 'points_SMA5', 'points_EMA5',
					'usageRate_EMA10', 'usageRate_SMA10', 'usageRate_EMA5',
					'usageRate_SMA5', 'numMinutes_EMA10', 'numMinutes_SMA10',
					'freeThrowsPercentage_EMA10', 'numMinutes_SMA5', 'numMinutes_EMA5',
					'turnovers_EMA10', 'freeThrowsPercentage_SMA10', 'turnovers_SMA10',
					'turnovers_EMA5', 'turnovers_SMA5', 'assists_EMA10',
					'freeThrowsPercentage_EMA5', 'freeThrowsPercentage_SMA5',
					'assists_SMA10', 'assists_EMA5', 'assists_SMA5',
					'threePointersAttempted_EMA10', 'threePointersAttempted_SMA10',
					'threePointersAttempted_SMA5', 'threePointersAttempted_EMA5',
					'threePointersMade_SMA10', 'threePointersMade_EMA10']

		elif market == 'player_assists':
			features = ['assists_EMA10', 'assists_SMA10', 'assists_EMA5',
					'assists_SMA5', 'turnovers_EMA10', 'turnovers_SMA10', 'turnovers_EMA5',
					'turnovers_SMA5', 'points_SMA10', 'usageRate_EMA10', 'points_EMA10',
					'usageRate_SMA10', 'usageRate_SMA5', 'points_SMA5', 'usageRate_EMA5',
					'points', 'points_EMA5', 'numMinutes_EMA10', 'numMinutes_SMA10',
					'numMinutes_SMA5', 'threePointersAttempted_SMA5', 'numMinutes_EMA5',
					'threePointersAttempted_SMA10', 'threePointersAttempted_EMA10',
					'threePointersAttempted_EMA5', 'freeThrowsPercentage_SMA10',
					'freeThrowsPercentage_EMA10', 'threePointersMade_SMA10',
					'threePointersMade_SMA5', 'threePointersMade_EMA10']

		elif market == 'player_threes':
			features = ['threePointersAttempted_SMA10', 'threePointersAttempted_EMA10',
					'threePointersAttempted_SMA5', 'threePointersAttempted_EMA5',
					'threePointersMade_SMA10', 'threePointersMade_EMA10',
					'threePointersMade_SMA5', 'threePointersMade_EMA5', 'points_SMA5',
					'points_EMA10', 'points_SMA10', 'numMinutes_SMA5', 'points_EMA5',
					'numMinutes_EMA10', 'numMinutes_SMA10', 'numMinutes_EMA5',
					'reboundsOffensive_EMA10', 'assists_SMA5', 'assists_EMA5',
					'reboundsOffensive_SMA10', 'assists_EMA10', 'reboundsOffensive_EMA5',
					'assists_SMA10', 'usageRate_EMA10', 'usageRate_SMA5', 'usageRate_EMA5',
					'usageRate_SMA10', 'turnovers_EMA5', 'turnovers_SMA5',
					'turnovers_EMA10']

		elif market == 'player_rebounds':
			features = ['reboundsTotal_EMA10', 'reboundsTotal_SMA10',
					'reboundsTotal_EMA5', 'reboundsTotal_SMA5', 'totalReboundingRate_SMA10',
					'totalReboundingRate_EMA10', 'totalReboundingRate_EMA5',
					'totalReboundingRate_SMA5', 'reboundsOffensive_SMA10',
					'reboundsDefensive', 'reboundsOffensive_EMA10',
					'reboundsOffensive_SMA5', 'totalReboundingRate',
					'reboundsOffensive_EMA5', 'fieldGoalsPercentage_SMA10', 'blocks_EMA10',
					'fieldGoalsPercentage_EMA10', 'blocks_SMA10', 'blocks_SMA5',
					'numMinutes_SMA10', 'numMinutes_EMA10', 'blocks_EMA5', 'points_SMA10',
					'numMinutes_SMA5', 'points_EMA10', 'numMinutes_EMA5', 'points_SMA5',
					'fieldGoalsPercentage_SMA5', 'points_EMA5', 'fieldGoalsPercentage_EMA5']

		elif market == 'player_steals':
			features = ['steals_EMA10', 'steals_SMA10', 'steals_EMA5',
					'opponentTeamFieldGoalsPercentage_SMA10', 'opponentTeamBlocks_EMA5',
					'opponentTeamFieldGoalsMade_SMA10', 'steals_SMA5',
					'opponentTeamBlocks_EMA10', 'opponentTeamFieldGoalsMade_EMA10',
					'opponentTeamBlocks_SMA5', 'opponentTeamFieldGoalsPercentage_EMA10',
					'opponentTeamFieldGoalsMade_SMA5', 'opponentTeamBlocks_SMA10',
					'opponentTeamFieldGoalsMade_EMA5', 'opponentTeamPlusMinusPoints_SMA5',
					'opponentTeamReboundsDefensive', 'opponentTeamPlusMinusPoints_SMA10',
					'opponentTeamFieldGoalsPercentage_SMA5',
					'opponentTeamTotalReboundingRate',
					'opponentTeamReboundsOffensive_SMA10', 'plusMinusPoints_SMA5',
					'opponentTeamPlusMinusPoints_EMA10',
					'opponentTeamFieldGoalsPercentage_EMA5', 'totalReboundingRate_SMA10',
					'opponentTeamFieldGoalsAttempted_SMA5', 'plusMinusPoints_EMA10',
					'threePointersMade_SMA5', 'opponentDefensiveRating_SMA5',
					'assists_EMA5', 'opponentTeamFieldGoalsAttempted_EMA10']

		elif market == 'player_blocks':
			features = ['blocks_EMA10', 'blocks_SMA10', 'blocks_SMA5', 'blocks_EMA5',
					'assists_EMA5', 'assists_EMA10', 'assists_SMA5', 'assists_SMA10',
					'opponentTeamTotalReboundingRate', 'trueShooting_EMA10',
					'trueShooting_EMA5', 'turnovers_EMA5', 'effectiveFieldGoal_EMA10',
					'steals_SMA10', 'turnovers_EMA10', 'turnovers_SMA5',
					'effectiveFieldGoal_EMA5', 'points', 'usageRate_SMA5',
					'usageRate_EMA5', 'usageRate_EMA10', 'restDays',
					'trueShooting_SMA10', 'steals_EMA10',
					'opponentTeamTotalReboundingRate_EMA10', 'usageRate_SMA10',
					'opponentTeamTotalReboundingRate_SMA5', 'trueShooting_SMA5',
		...
		print('sport title: {}, not known , no custom feature selection was done to subset'.format(sport))
		return None

def evaluate_predictions(df, line_col='line', predicted_line_col='predictedLineOutCome',  actual_stat_col='outcomeStat',
                         prop_type_col='overUnder', result_col='predictionCorrect'):
    """
    ...
    if prop_type_col == 'overUnder':
        ...
        def check_correct(row):
            if pd.isnull(row[predicted_line_col]) or pd.isnull(row[line_col]) or pd.isnull(row[actual_stat_col]):
                return np.nan
            if (row[predicted_line_col] == 1) and (row[prop_type_col] == 'Over'):  # predicted Over
                return row[actual_stat_col] > row[line_col]
            elif row[predicted_line_col] == 0 and (row[prop_type_col] == 'Over'):  # predicted Over
                return row[actual_stat_col] < row[line_col]
            elif (row[predicted_line_col] == 1) and (row[prop_type_col] == 'Under'):  # predicted Under
                return row[actual_stat_col] < row[line_col]
            elif row[predicted_line_col] == 0 and (row[prop_type_col] == 'Under'):  # predicted Under
                return row[actual_stat_col] > row[line_col]
            else:
                return np.nan
...
def format_email_body(df):
    ...
    for event_id in df['eventId'].unique():
        ...
        match_up = "{} at {}".format(game_df['awayTeam'].iloc[0], game_df['homeTeam'].iloc[0])

        game_df['take'] = game_df.apply(lambda row: (
                'Over' if row['overUnder'] == 'Over' and row['predictedLineOutCome'] == 1 else
                'Under' if row['overUnder'] == 'Over' and row['predictedLineOutCome'] == 0 else
                'Under' if row['overUnder'] == 'Under' and row['predictedLineOutCome'] == 1 else
                'Over'), axis=1)
...
def get_high_accuracy_player_market(rate=.7, look_back_weeks=4):
    '''Returns a list of players/market with an accuracy rate that meets the applied threshold'''

    # Read open latest results file
    results = pd.read_parquet('./outcomes/results/basketball_nba/basketball_nba_2025.parquet', engine='pyarrow')

    # Filter for past 4 weeks
    trim_date = (datetime.now() - relativedelta(weeks=look_back_weeks)).date()
    trim_df = results[results['eventDate'] >= trim_date].copy()

    # Accuracy rate list
    acc_df = trim_df.groupby(['personId', 'market']).agg({'predictionCorrect': 'sum', 'eventId': 'count'})
    acc_df = acc_df.reset_index()
    acc_df['predictionCorrect'] = acc_df['predictionCorrect'].apply(lambda x: (1 if x == True else 0 if x == False else x))
    acc_df['acc_rate'] = acc_df['predictionCorrect']/acc_df['eventId']
    acc_df['personId_market'] = acc_df['personId'].astype(str) + "-" + acc_df['market']

    # create list
    acc_df = acc_df[acc_df['acc_rate'] >= rate].copy()
    high_acc_list = acc_df['personId_market'].to_list()

    return high_acc_list

def email_betting_odds(df):

	msg = EmailMessage()
	recipients = ["javolwilloughby@gmail.com", "Guillermoulloa14@gmail.com", "Gpinales15@gmail.com"]
...
def collect_and_merge_predictions_and_outcomes(stats_df, sport):
	...
	try:
		...
		predictions = predictions.sort_values('file_name', ascending=False).drop_duplicates(['eventId', 'personId', 'market', 'overUnder']).reset_index(drop=True)
		predictions['predictedStat'] = predictions[['predictedPoints', 'predictedAssists', 'predictedThrees', 'predictedRebounds',
		                                            'predictedSteals', 'predictedBlocks',]].bfill(axis=1).iloc[:, 0]
		predictions = predictions.drop(['predictedPoints', 'predictedAssists','predictedThrees', 'predictedRebounds', 'predictedSteals',
		                                'predictedBlocks', 'file_name', 'file_date',  'nextGamePoints', 'nextGameAssists', 'nextGameThreePointersMade',
		                                'nextGameReboundsTotal', 'nextGameSteals', 'nextGameBlocks'], axis=1)
		...
		stats_cols = ['points', 'threePointersMade', 'assists', 'reboundsTotal', 'steals', 'blocks']
		...
		long_df['market'] = long_df['market'].replace({'points': 'player_points', 'threePointersMade': 'player_threes', 'assists': 'player_assists',
		                                                'reboundsTotal': 'player_rebounds', 'steals': 'player_steals', 'blocks': 'player_blocks'
		                                                })
		long_df = long_df.rename(columns={'gameDateUTC': 'eventDateTime'})

		predictions_outcomes = pd.merge_asof( predictions.sort_values('eventDateTime'), long_df.sort_values('eventDateTime'),
		                        left_on='eventDateTime', right_on='eventDateTime', by=['personId', 'market'],
...
def check_sports_schedule(sport):
	...
	if sport == 'basketball_nba':
		...
		nba_schedule['gameDate'] = nba_schedule['gameDateTimeEst'].apply(parse_game_date).dt.date
		todays_schedule = nba_schedule[nba_schedule['gameDate'] == datetime.now(ZoneInfo("America/New_York")).date()]
...
def collect_all_player_props():
	...
	all_files = [f for f in all_files if 'historical_player_props_basketball_nba_20250924_193509.parquet' not in f]
	...
	props = props.sort_values('timeStamp', ascending=False)\
			.drop_duplicates(['eventId', 'fullNameId', 'market', 'overUnder'])\
			.reset_index(drop=True)

	return props

def create_isotonic_regressor_calibrator_model (stats, props_df, market, target, xgb_progressive_model, sport, model_type, stat_type):
	...
	props_and_box = pd.merge_asof(props_df.sort_values('eventDateTime'), stats.sort_values('nextGameDateUTC'),
	...
	props_and_box = props_and_box[(props_and_box['market'] == market) & (props_and_box['eventDate'] >= (datetime.now(tz=pytz.utc) - relativedelta(weeks=8)).date())].copy()
	...
	os.makedirs(f'./models/{sport}/xgboost_regressor_progressive_isotonic_calibrations', exist_ok=True)
	joblib.dump(bundle, f'./models/{sport}/xgboost_regressor_progressive_isotonic_calibrations/xgb_progessive_isotonic_{stat_type}_model_{datetime.now().date().strftime("%Y%m%d")}.pkl')

	print(f"✅ Isotonic calibration model created and saved for {sport} - {market} ({stat_type}).")

def update_xgb_progressive_models (stats_df, props_df, market, stat_type, model_type, target, sport, current_xgb_model, og_model_path, force_start_date=None):

	# Select last week’s data (if applicable)
	if force_start_date == None:
		if os.path.exists(f'./models/{sport}/model_update_tracker.csv'): # if tracker file exists, check there first
			tracker_file = pd.read_csv(f'./models/{sport}/model_update_tracker.csv')
			if tracker_file[tracker_file['model'] == f'progressive_{stat_type}']['last_update'].iloc[0] != None: # Check if value exists for model
				tracker_file['last_update'] = pd.to_datetime(tracker_file['last_update']).dt.date
				start_date = tracker_file[tracker_file['model'] == f'progressive_{stat_type}']['last_update'].iloc[0]
			else:
	...
	props_and_box = pd.merge_asof(props_df.sort_values('eventDateTime'), stats_df.sort_values('nextGameDateUTC'),
								left_on='eventDateTime', right_on='nextGameDateUTC', by=['fullNameId'],
								tolerance=pd.Timedelta('2h'), direction='nearest')
	...
	if not market_df.empty:
		...
		x_train, x_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=10) # Split, train, test
		...
		update_xgb_model = copy.deepcopy(current_xgb_model) # <-- deep copy to avoid having the same model in memory
		update_xgb_model = update_xgb_model.fit(x_train, y_train, xgb_model=current_xgb_model.get_booster())
		...
		if update_rmse <= current_rmse:
			...
			final_model = update_xgb_model.fit(X, y, xgb_model=update_xgb_model.get_booster()) # Update progressive learner

			# Save
			os.makedirs(f'./models/{sport}/xgboost_regressor_{model_type}_calibrations', exist_ok=True)
			...
			if os.path.exists(f'./models/{sport}/model_update_tracker.csv'):
				...
				tracker.loc[tracker["model"] == f'progressive_{stat_type}', 'last_update'] = latest_fitted_date
				tracker.to_csv(f'./models/{sport}/model_update_tracker.csv', index=False)
			else:
				latest_fitted_date = market_df['eventDateTime'].max()
				# create tracker file
				model_stats = ['progressive_points', 'progressive_assists', 'progressive_threes', 'progressive_rebounds', 'progressive_steals', 'progressive_blocks']
			...
			create_isotonic_regressor_calibrator_model(stats=stats_df, props_df=props_df, market=market, target=target,
														sport=sport, xgb_progressive_model=final_model, model_type=model_type,
														stat_type=stat_type)

	else:
		print(f'Dataset for {sport}, {market} for date start at {start_date} is empty... No models updated for today')
...
def prop_model_evaluation (model, model_type, model_path, stat_type, props_box_df, sport):
    """
...
def get_player_images(fullNameId):
	...
	for file_name in image_folder:
	    full_name_id = file_name[:-4].upper().replace('.', '').replace(' ', '').replace('-', '')
